...
The `preToolUse` hook receives JSON input with details about the tool being called. Your script can inspect this input and exit with a specific code to control the outcome:

| Exit code | Outcome |
|-----------|---------|
| `0` | **Approve** the tool execution |
| `1` (or any non-zero except 2) | **Deny** the tool execution with a generic error |
| `2` | **Deny** the tool execution (v1.0.70+) — same effect as code 1, but reserved for explicit deny. Use this to clearly distinguish a deliberate policy denial from an accidental script error. |

> **v1.0.70+ exit code 2**: Prior to v1.0.70, any non-zero exit code denied the tool call. From v1.0.70, exit code `2` is the canonical way to issue an explicit deny from a `preToolUse` hook. Using exit code `2` makes your hook's intent unambiguous when reading logs or debugging.
