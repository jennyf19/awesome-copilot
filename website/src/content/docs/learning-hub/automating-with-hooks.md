...
> **Exit code 2 — explicit deny (v1.0.70+)**: Exiting with code `2` is a guaranteed deny signal. Unlike other non-zero codes (which may be interpreted as a general failure), code `2` unambiguously signals that the hook intentionally blocked the tool call. Use exit code `2` in security gates and policy scripts to make the intent clear and ensure the tool is denied even if hook-error handling changes in future releases.
