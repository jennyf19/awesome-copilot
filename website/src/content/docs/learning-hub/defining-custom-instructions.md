...
In GitHub Copilot CLI (v1.0.81-6+), the `/instructions` slash command lists each loaded instruction file individually, so you can verify exactly which files are active in the current session.
