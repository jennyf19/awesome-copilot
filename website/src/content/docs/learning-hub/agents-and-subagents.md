...
> **Default depth lowered in v1.0.71+**: The default maximum subagent nesting depth was reduced from **6 to 4** to curb runaway recursive subagent delegation. Most workflows are unaffected — typical orchestration rarely goes deeper than 2 or 3 levels. If your workflow requires deeper nesting, usage-based billing users can increase `subagents.maxDepth` up to 128 in `/settings`.
