...
Starting with v1.0.72, **multi-turn subagents are always enabled** in Copilot CLI. This means you can send follow-up messages to a running subagent — for example, to steer it mid-task, provide clarification, or ask it to adjust its approach — without waiting for the subagent to finish and report back.

Previously this required an experimental feature flag. It is now on by default for all sessions.
