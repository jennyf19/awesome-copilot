...
### Opening the App from Copilot CLI *(v1.0.81+)*

If you use GitHub Copilot CLI alongside the Copilot app, you can open the current session's directory in the app directly from your terminal:

```bash
copilot app
```

This opens the GitHub Copilot app focused on the current working directory (requires GitHub Copilot app 1.1.3 or later). Use it as a quick bridge between a CLI session and the app's graphical interface — for example, to hand off a task to the app's visual parallel-work view.
