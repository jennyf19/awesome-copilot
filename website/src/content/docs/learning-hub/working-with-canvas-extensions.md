...
Canvas extensions give you shared, interactive work surfaces inside the GitHub Copilot app and the GitHub Copilot CLI (v1.0.71+). Instead of keeping all progress in chat, you can move work into a visible artifact (such as a board, document, checklist, or browser-oriented surface) that both people and agents can update.
...
> **Canvas support in the CLI (v1.0.71+)**: Canvases are now available in the GitHub Copilot CLI in addition to the Copilot app. Extension-driven canvases can be used directly from your terminal when a canvas extension is installed. This enables the same bidirectional canvas patterns in CLI workflows and automation pipelines.
