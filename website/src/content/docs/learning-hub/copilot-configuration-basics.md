...
| `githubMcpToolsets` | Persist GitHub MCP toolset configuration across sessions (v1.0.71+) |
| `githubMcpTools` | Persist GitHub MCP tool enable/disable configuration across sessions (v1.0.71+) |
| `pinnedPrompts` | Control prompt pinning behavior in sessions (v1.0.71+) |
...
**Repository-pinned model settings** (v1.0.70+): A trusted repository can enforce the model, reasoning effort level, and context tier for everyone working in it by adding a `.github/copilot/settings.json` file:
...
These settings override a user's own model preference for that repository, making it easy to ensure team members use the right model for latency-sensitive or cost-sensitive projects. The file also lets you extend the URL, MCP server, and skill deny lists at the repository level.
