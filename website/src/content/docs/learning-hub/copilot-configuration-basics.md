...
| `defaultPermissionMode` | Default approval behavior for new interactive sessions (e.g., `auto-approve`, `manual`) (v1.0.81+) |
...
**Adaptive model selection in Auto mode** (v1.0.81+): Auto mode now continuously adapts model selection as your task evolves during a conversation. As the context and complexity of your work changes — for example, moving from a simple question to a deep refactoring task — Auto mode recalibrates the model choice, ensuring you always benefit from the most appropriate model at each stage without any manual intervention.
...
**Session restore on startup** (v1.0.81+): If the CLI exits unexpectedly — due to a crash or machine restart — it now offers to restore any sessions that were still open when it closed. Instead of losing your work context, you can pick up right where you left off without manually reopening each terminal session.
...
**Voice dictation** (v1.0.81+): Use **Ctrl+Space** to toggle voice dictation for hands-free prompt entry. This lets you speak your request instead of typing it — useful when your hands are occupied or you prefer a conversational input style.
