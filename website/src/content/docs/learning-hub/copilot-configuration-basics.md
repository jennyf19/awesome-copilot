...
| `defaultPermissionMode` | Choose the default approval behavior for new interactive sessions (`auto`, `allow-all`, `manual`) (v1.0.81+) |
...
The `--add-dir` flag *(v1.0.81+)* adds an extra directory from which Copilot CLI discovers custom agents and skills. Skills and agents in the specified directory are loaded alongside those from your repository and user configuration:

```bash
copilot --add-dir ~/shared-agents     # load agents and skills from a shared directory
```

This is useful for teams that maintain a central set of agents and skills outside any single repository — for example, a shared company-wide agents directory that everyone loads without installing a plugin.

**Startup session restore** *(v1.0.81+)*: When Copilot CLI starts, it offers to restore any sessions that were still open when a previous CLI instance exited unexpectedly (e.g., due to a crash or machine restart). This means you no longer lose in-progress sessions to unexpected shutdowns.
